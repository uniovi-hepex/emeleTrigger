...
The CLI wrapper in optim/cli.py exposes the same functionality from shell:
...
def _objective(trial: optuna.Trial, ds: OMTFDataset) -> float:
    ...
    metrics = trainer.fit(train_loader, val_loader)  # returns dict with "mse"
    log.info("trial %s → %.5f", trial.number, metrics["mse"])
    return metrics["mse"]  # Optuna minimises by default
...
def run_study(graphs_pt: str | Path, n_trials: int = 30,
    ...
    study.optimize(lambda t: _objective(t, ds), n_trials=n_trials, timeout=None)
