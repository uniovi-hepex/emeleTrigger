...
class OMTFGraphNetwork():
    ...
    def load_data(self, path,fraction=1., viz=False, bsize=64):
        ...
        self.train_dataset = self.pg_graphs[:int(72428/2)]
        self.test_dataset  = self.pg_graphs[int(72428/2):] #TO DO fix this
    ...
    def _get_stub_r(self, stubTypes, stubEtas, stubLayer, stubQuality):
      rs=[]
    ...
    def addEdges(self):
        ...
        for index, cloud in enumerate(self.sky): #index -> one per event 
            graph = nx.DiGraph()
            nodes = []
            keep=['stubR', 'stubEta', 'stubPhi', 'stubEtaDist', 'stubPhiDist', 'stubQuality', 'stubBx', 'stubDetId', 'stubType', 'stubTiming', 'stubLayer'] #remove detID
            edges=[]
            #print("index",index,cloud["eventNum"])
            ...
            for idx, row in cloud.iterrows(): # build edges based on stubLayer
                ...
                dests=self.getEdgesFromLogicLayer(row['stubLayer'])

                #print(dests,idx)
                for queriedindex, row in cloud.iterrows():
                    #print(row['stubLayer'])
                    if row['stubLayer'] in dests:
                        #print("pass",(idx, queriedindex,row['stubLayer']))
    ...
    def convert_to_graphs(self, viz=False):
        ...
        keep=['stubR','eventNum', 'stubEta', 'stubPhi', 'stubEtaDist', 'stubPhiDist', 'stubQuality', 'stubBx', 'stubDetId', 'stubType', 'stubTiming', 'stubLayer', 'muonPt']
        self.sky=[]
        ...
        print('original dframe had size', len(self.branches))    
